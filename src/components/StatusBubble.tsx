...
function cloudPath(w: number, h: number, padX: number, padTop: number, padBottom: number): string {
  const radii = [14, 8, 13, 7, 15, 8, 12, 7];
  ...
  function edgeBumps(len: number, startX: number, startY: number, dx: number, dy: number, ri: number): string {
    const n = Math.max(2, Math.floor(len / 20));
    const step = len / n;
    let d = "";
    for (let i = 0; i < n; i++) {
  ...
  let ri = 0;
  const topN = Math.max(2, Math.floor(topLen / 20));
  ...
  const rightN = Math.max(2, Math.floor(rightLen / 20));
  ...
  const bottomN = Math.max(2, Math.floor(bottomLen / 20));
  ...
  const leftN = Math.max(2, Math.floor(leftLen / 20));
